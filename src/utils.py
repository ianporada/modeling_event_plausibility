...
def collate_training_batch(batch):
    """Flatten and collate."""
    return collate_batch([item for items in batch for item in items])


class PreprocessingModule:
    """Tokenizes and preprocesses input examples."""
    ...
    def load_dicts(self):
        """Load WordNet as dictionaries data from .tsv files."""
        ...
        # Wordnet data is filtered and stored in these tsv files.
    ...
    def item_to_features(self, *args):
        ...
        elif self.model_type == 'conceptinject':
            return self.item_to_features_conceptinject(*args)
        else:
    ...
    def item_to_features_conceptmax(self, subj, verb, obj, subj_hyp, obj_hyp, label, training):
        ...
        output['lengths'] = len(texts)
        
        return output
    
    def item_to_features_conceptinject(self, subj, verb, obj, subj_hyp, obj_hyp, label, training):
        texts = [' '.join([subj, verb, obj])]
        features = self.tokenizer(
            texts, max_length=16, padding='max_length', truncation=True,
            return_tensors='pt'
    ...
    def synset_to_hc(self, synset, lemma):
        """Get the hypernym chain for a given synset."""
        ...
        lemma_hc = [x.replace('_', ' ') for x in lemma_hc]
